...
import { DecodedUserJwtPayload, JwtTokens, RefreshToken, UserJwtPayload } from '../types';
...
export class JWTService {
  ...
  verifyRefreshToken = (token: string): DecodedUserJwtPayload => {
    const options = { secret: this.envConfigService.getJwtRefreshTokenSecret() };
    const payload = this.jwtService.verify<DecodedUserJwtPayload>(token, options);
